...
from components.lights import GoveeMqttLight
...
def light(mock_logger, mock_mqtt):
    return GoveeMqttLight(mock_logger, mock_mqtt, "test_govee", "govee/light", "govee/light/set")

class TestGoveeMqttLight:
    def test_turn_on_increments_counter_and_delays_update(self, light, mock_mqtt):
        ...
        expected_payload = json.dumps({"state": "ON", "brightness": 100})
        mock_mqtt.mqtt_publish.assert_called_once_with("govee/light/set", expected_payload)
    ...
    def test_mqtt_message_updates_state_and_calls_listener(self, light):
        ...
        # When
        payload_on = json.dumps({"state": "ON", "brightness": 100})
        light.on_mqtt_message("MQTT_MESSAGE", {"payload": payload_on}, {})
        
        # Then
        assert light.is_on() is True
        listener_mock.assert_called_once_with("test_govee", True)
